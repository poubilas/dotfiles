...
class Pager:
    ...
    def __init__(self, content: str, lines_per_page: int = None, is_markdown: bool = False):
        ...
        self.lines_per_page = lines_per_page or (console.height - 3)  # Mehr Zeilen nutzen
        ...
        while True:
            try:
                ...
                if "OSS 120B" in self.current_model or "oss 120b" in self.current_model.lower():
                    prompt_text += f" | reasoning: {self.reasoning_effort}"
                prompt_text += "[/dim cyan] [bold]>[/bold]"

                # Verwende input() statt Prompt.ask() für bessere readline-Kompatibilität
                # Zeige Prompt mit Rich-Formatierung
